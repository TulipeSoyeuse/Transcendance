...
export async function googleAuthCallback(fastify) {
    fastify.post('/auth/google/callback', async (request, reply) => {
        ...
        try {
            ...
            let user = await fastify.database.fetch_one('SELECT * FROM user WHERE google_id = ?', [googleId]);
            if (!user) {
...
export function whoami(fastify) {
    return async function (request, reply) {
        if (!request.session.authenticated) {
            return reply.status(401).send({ error: "Not authenticated" });
        }
        try {
            const rows = await fastify.database.fetch_all("SELECT id, username, email, is_admin FROM user WHERE id = ?", [request.session.userId]);
            if (!rows.length) {
