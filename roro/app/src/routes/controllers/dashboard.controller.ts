...
export function getGameHistory(fastify: FastifyInstance) {
	return async function (request: FastifyRequest, reply: FastifyReply) {
		try {
		  const allGames = await fastify.database.fetch_all(
			`SELECT * FROM match
			WHERE (player_1 = ? OR player_2 = ?)
			ORDER BY id ASC`,
			[request.session.userId]
		  );
		  if (!allGames) return (reply.status(404).send({ message: "No games played" }));
